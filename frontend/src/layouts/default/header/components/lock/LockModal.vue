...
<script lang="ts" setup>
  ...
  // 用户头像：userInfo.avatar 优先（后端将来可提供真实头像），缺省用 DiceBear 生成
  // （utils/avatar 统一生成：与头部用户标签、解锁页同一用户头像一致）
  const avatar = computed(() => {
    const { avatar: userAvatar } = userStore.getUserInfo;
    return userAvatar || createAvatarUrl(getRealName.value);
  });
</script>
<style lang="less">
