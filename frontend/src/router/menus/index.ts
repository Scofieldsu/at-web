...
function basicFilter(routes: RouteRecordNormalized[]) {
  return (menu: Menu) => {
    // 外链菜单没有对应路由，点击由 openWindow 接管（见 layouts/default/menu）。
    // 必须直接放行：原实现让它匹配到 routes[0] 并把那条路由的 meta 覆盖上来，
    // 外链会显示成别的菜单的标题和图标。
    // ROLE 模式下菜单项是原始路由对象（只有 meta.icon，没有顶层 icon），而
    // MenuItemContent 读的是 item.icon，所以这里要自己把图标提上来。
    if (isHttpUrl(menu.path)) {
      menu.icon = (menu.icon || menu.meta?.icon) as string;
      return true;
    }
