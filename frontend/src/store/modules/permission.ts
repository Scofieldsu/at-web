    ...
    async buildRoutesAction(): Promise<AppRouteRecordRaw[]> {
      ...
      switch (permissionMode) {
        ...
        case PermissionModeEnum.ROLE:
          ...
          // ROLE 模式下菜单项是原始路由对象，orderNo 声明在 meta 里（本项目全部
          // 模块都是如此），原来只读顶层 a.orderNo 时全为 undefined，排序等于没做，
          // 实际顺序由 glob 的文件名顺序决定（external.ts 排在 platform.ts 前，外部系统
          // 就跑到了最上面）。这里按 meta.orderNo 兜底顶层，声明的顺序才真正生效。
          staticMenuList.sort((a, b) => {
            const ao = a.meta?.orderNo ?? a.orderNo ?? 0;
            const bo = b.meta?.orderNo ?? b.orderNo ?? 0;
            return ao - bo;
          });
          // 设置菜单列表
